...
$description_bottom = '<p>После заполнения всех полей и оплаты, документ будет полностью сформирован и отправлен в форме файла .pdf на указанный Вами e-mail.</p>
...
$description_modal_bottom = '<p>После оплаты документ будет полностью сформирован, файл с текстом документа отправлен на указанный Вами e-mail, также будет доступен для скачивания в <a href="<?php echo $link_lk;?>" target="_blank">Личном кабинете</a>.</p>';
$document_drafting = config('main', 'document_drafting');
if (in_array($data['product']['id'], $document_drafting)) {
    $description_bottom = '<p>После заполнения всех полей и оплаты, юрист приступит к работе. В ходе работы над документом юрист свяжется с Вами для уточнения деталей.</p>
    <p>После завершения работы юриста, документ будет выслан Вам на почту.</p>';
    $description_modal_bottom = '<p>После оплаты, юрист приступит к работе. В ходе работы над документом юрист свяжется с Вами для уточнения деталей.</p>
    <p>После завершения работы юриста, документ будет выслан Вам на почту.</p>';
}
?>
<div class="container p-4 py-md-5">
    ...
    <div class="d-flex gap-2 product-btn">
        <button id="btn-fields"
            class="btn btn-success mt-2 mt-md-0 d-inline-block d-lg-none"
            type="button"
            onclick="btnFields(this)">Редактировать шаблон</button>
    ...
    <div class="fields-list-row mt-3">
        <div class="col-lg-4 d-lg-flex fields-list-col">
            <button type="button" class="btn-close text-reset btn-close-custom" onclick="btnFieldsClose(this)"></button>
            <div class="bg-white p-3 py-md-4 rounded w-100 fields-list-col-wrap">
...
<div id="warning-form-modal" class="modal fade" tabindex="-1" aria-labelledby="warning-form-modal-modalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-success" data-bs-dismiss="modal" onclick="fillFields()">Заполнить поля</button>
            ...
            <div class="modal-body mt-3 mb-3">
                ...
                <form id="buy_doc_form">
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3">
                            <?php echo $description_modal_bottom;?>
                        </div>
                    </div>
                    <?php echo csrf_field();?>
...
<div class="overlay-fields fade"></div>
